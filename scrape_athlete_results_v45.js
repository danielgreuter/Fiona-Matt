...
async function buildCompIndex(page, oldestISO) {
  ...
  for (let pg = 1; pg <= MAX_PAGES; pg++) {
    ...
    let pageMax = '0000-00-00';   // neuestes Datum der Seite
    for (const r of rows) {
    ...
    // Abbruch erst, wenn die GANZE Seite älter als nötig ist (robust gegen
    // einzelne alt-datierte Ausreisser, die nur pageMin verfälschen würden)
    if (pageMax < oldestISO) break;
  }
  return index;
}
...
function matchComps(result, index) {
  ...
  return cand.map(c => ({ c, s: score(c) }))
             .sort((a, b) => b.s - a.s)
             .map(x => x.c)
             .slice(0, 12); // pro Tag höchstens 12 Meetings probieren
}
