// scrape_athlete_results_v18.js
// Fix: Playwright page.click() für loadDataBtn (nicht JS element.click())
// Button-ID jetzt bekannt: form_anonym:loadDataBtn
...
async function parseTable(page, discLabel, year) {
  ...
  const info = await page.evaluate(() => {
    ...
    return {
      count: trs.length,
      first3: trs.slice(0,3).map(r => r.textContent.trim().replace(/\s+/g,' ').slice(0,100)),
    };
  });
  console.log(`    → ${info.count} Zeilen`);
  if (info.count > 2) console.log(`    [0]: ${info.first3[1]||''}`);
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { season, disc } of COMBOS) {
      ...
      // Anzeigen — Playwright click (nicht JS element.click)!
      try {
        await page.click('#form_anonym\\:loadDataBtn', { timeout: 8000 });
