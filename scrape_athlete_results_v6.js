...
// ── Step 1: Search for Fiona, get alabus athlete page URL ─────

async function findFionaUrl() {
  // GET alabus search page
  ...
  // Find submit button
  const btnMatch = html.match(/id="(form_anonym:[^"]*(?:suche|search|idt)[^"]*)"[^>]*(?:type="submit"|class="[^"]*btn)/i)
    || html.match(/<input[^>]*type="submit"[^>]*id="(form_anonym:[^"]+)"/i);
  const searchBtn = btnMatch ? btnMatch[1] : 'form_anonym:j_idt_search';
  ...
  const resultHtml = r.html || r.xml;

  // Strategie 1: <a onclick="openURLForBestlist('URL')">Fiona Matt</a>
  let saUrl = null;
  const s1 = resultHtml.match(/openURLForBestlist\('([^']+)'\)[^>]*>\s*Fiona\s+Matt\s*</i);
  if (s1) saUrl = s1[1];

  // Strategie 2: "Fiona Matt" gefolgt von athlete-URL
  if (!saUrl) {
    const s2 = resultHtml.match(/Fiona\s+Matt[\s\S]{0,300}?openURLForBestlist\('([^']+(?:ath|athlet|person|pro)[^']+)'\)/i);
    if (s2) saUrl = s2[1];
  }

  // Strategie 3: athlete-URL gefolgt von Fiona
  if (!saUrl) {
    const s3 = resultHtml.match(/openURLForBestlist\('([^']+(?:ath|athlet|pro-athlet)[^']+)'\)[^>]*>[\s\S]{0,100}?Fiona/i);
    if (s3) saUrl = s3[1];
  }

  // Strategie 4: alle URLs, erste die nicht "verein" oder "verband" enthält
  if (!saUrl) {
    const all = [...resultHtml.matchAll(/openURLForBestlist\('([^']+)'\)/g)];
    const found = all.find(m => !m[1].includes('verein') && !m[1].includes('verband'));
    if (found) saUrl = found[1];
  }

  if (!saUrl) {
    console.log('   DEBUG (erste 3000):', resultHtml.substring(0, 3000));
    throw new Error('Fiona URL nicht gefunden');
  }

  if (!saUrl.startsWith('http')) saUrl = 'https://www.swiss-athletics.ch' + saUrl;
  console.log('   SA-URL:', saUrl);

  // Fetch swiss-athletics.ch Athleten-Seite → alabus iframe finden
  const saRes = await fetch(saUrl, { headers: { 'User-Agent': UA } });
  const saHtml = await saRes.text();

  const iframeMatch = saHtml.match(/src="(https:\/\/alabus\.swiss-athletics\.ch[^"]+bestlistathlete[^"]+)"/i)
    || saHtml.match(/(?:iframe|frame)[^>]+src="([^"]*alabus[^"]+)"/i);

  if (!iframeMatch) {
    // Möglicherweise relative URL oder JS-geladen — versuche direkte alabus-URL zu konstruieren
    const athMatch = saUrl.match(/ath=([^&]+)/);
    if (athMatch) {
      const alabusUrl = BASE + '/satweb/faces/bestlistathlete.xhtml?externalKey=' + athMatch[1] + '&lang=de';
      console.log('   Alabus-URL (konstruiert):', alabusUrl);
      const ar = await fetch(alabusUrl, { headers: { 'User-Agent': UA } });
      const ah = await ar.text();
      const ac = ar.headers.get('set-cookie') || '';
      return { url: alabusUrl, session: (ac.match(/JSESSIONID=([^;]+)/) || [])[1] || session, guid: extractGuid(ah) };
    }
    console.log('   DEBUG SA-HTML (erste 2000):', saHtml.substring(0, 2000));
    throw new Error('Alabus iframe nicht gefunden');
  }

  const alabusUrl = iframeMatch[1].replace(/&amp;/g, '&');
  console.log('   Alabus-URL:', alabusUrl);
  ...
  return {
    url: alabusUrl,
    session: (ac.match(/JSESSIONID=([^;]+)/) || [])[1] || session,
    guid: extractGuid(ah),
  };
}
