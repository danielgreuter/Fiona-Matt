...
// Basis: v56 + v57: korrekter Parameter fuer windunterstuetzte Resultate ist sw=1 (nicht srb) - verifiziert an der Bestenlisten-URL
...
async function scrapeQuery(outer, inner, con, year, cat, disc, indoor, sw) {
  const saUrl = buildSaUrl(con, year, cat, disc, indoor, sw);
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { disc, indoor } of QUERIES) {
      ...
      // Outdoor-Sprints: zusaetzlich windunterstuetzte Resultate (sw=1) einsammeln.
      ...
      if (!indoor && /\dm$/.test(disc.replace(/\s+/g, ''))) {
        ...
        if (neu.length) {
          console.log(`    → +${neu.length} windunterstuetzt (sw=1)`);
