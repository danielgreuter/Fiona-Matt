...
async function scrapeDiscipline(page, disc, year) {
  ...
  if (rows.length === 0) {
    const allRows = await iframeFrame.evaluate(() => {
      const trs = document.querySelectorAll("table tr");
      const result = [];
      let i = 0;
      for (const tr of trs) { if (i++ > 4) break; const tds = tr.querySelectorAll("td"); if (tds.length > 0) result.push(Array.from(tds).map(td => td.textContent.trim())); }
      return { tableRows: result, bodySnippet: document.body.innerHTML.substring(0, 800) };
    });
    console.log("  DEBUG:", JSON.stringify(allRows, null, 2));
    return null;
  }
