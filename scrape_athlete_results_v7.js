#!/usr/bin/env node
// scrape_athlete_results_v7.js — System Chrome statt Chromium-Download
// Identisch mit v5, aber executablePath auf vorinstalliertes Chrome gesetzt
// → kein npx playwright install --with-deps nötig → kein Timeout
...
function parseRow(cells, combo) {
  ...
  // Rang-Parser: "5h2" → "5. Lauf 2", "3f" → "3. Final", "1c" → "1. C-Final"
...
// Finde vorinstalliertes Chrome auf GitHub Actions Runner
function findChrome() {
  const candidates = [
  ...
  return null; // Playwright nutzt dann eigenes Chromium
}

async function main() {
  console.log('🚀 Swiss Athletics Athleten-Resultate Scraper v7');

  const chromePath = findChrome();
  if (chromePath) console.log(`✅ System Chrome: ${chromePath}`);
  else console.log('⚠️  Kein System Chrome — nutze Playwright-Chromium');
  ...
  try {
    const btn = page.locator('button:has-text("Ja"), button:has-text("Akzeptieren"), button:has-text("Accept")').first();
    if (await btn.isVisible({ timeout: 3000 })) { await btn.click(); await page.waitForTimeout(1000); }
  } catch(e) {}

  // Suche nach Fiona Matt
  const searchFrame = page.frames().find(f => f.url().includes('bestlistathletesearch'));
  if (!searchFrame) throw new Error('Search frame nicht gefunden');
