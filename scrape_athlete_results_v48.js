...
// Basis: v47 + v48: robustes Past-Archiv-Parsing (Datum dd.mm.yyyy & Bereiche, tolerante Zeilen) + Diagnose-Logs
...
function parseAnyDate(s) {
  // 1) "30. Mai 2026" (auch "29. - 31. Mai 2026": erstes Tagespaar + Monat/Jahr)
  let m = String(s).match(/(\d{1,2})\.\s*(?:-\s*\d{1,2}\.\s*)?([A-Za-zäöüÄÖÜ]+)\s*(\d{4})/);
  if (m && DE_MONTHS[m[2]]) {
    return `${m[3]}-${String(DE_MONTHS[m[2]]).padStart(2,'0')}-${String(parseInt(m[1])).padStart(2,'0')}`;
  }
...
let LAP_BUDGET = 700;            // harte Obergrenze an laportal-Seitenabrufen
...
async function buildCompIndex(page, oldestISO) {
  const index = [];
  const MAX_PAGES = 90;        // Past hat 84 Seiten
  let emptyStreak = 0;
...
function candidatesFor(events, disc) {
  const tok = laportalDiscToken(disc).toLowerCase().replace(/\s+/g, '');
  const cands = events.filter(e => {
    const flat = (e.name || '').toLowerCase().replace(/\s+/g, '');
    if (!flat.startsWith(tok)) return false;
    if (/h\u00fcrden|huerden|hurdles/i.test(e.name)) return false;
    if (/staffel|relay/i.test(e.name)) return false;
    return true;
  });
  // Frauen-/Maedchen-Events zuerst probieren (spart Budget), Rest bleibt erlaubt
  const wRe = /frauen|damen|m\u00e4dchen|maedchen|girls|wom|u\d+\s*w|w\s*u\d/i;
  return cands.sort((a, b) => (wRe.test(b.name) ? 1 : 0) - (wRe.test(a.name) ? 1 : 0));
}
