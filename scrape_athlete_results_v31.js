// scrape_athlete_results_v31.js
// Fix: Name-Filter "Fiona Matt" — bltype=0 zeigt alle Athleten, wir filtern auf Fiona
...
// bltype=0: Alle Resultate (mehrere pro Athlet) → Fiona erscheint öfter → mehr Treffer
// bltype=1: Ein Resultat pro Athlet → Fiona als Rank-11 sicher dabei (aber nur Bestresultat)
// Strategie: beide abfragen, zusammenführen, deduplizieren
...
function buildUrl(year, cat, disc, indoor, bltype) {
  ...
  return rawRows
    ...
    .map(({ cells }) => {
      ...
      return {
        ...
        year: rowYear, place, source: 'swiss-athletics',
      };
    }).filter(Boolean);
}
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { disc, indoor } of COMBOS) {
      ...
      // Zwei Abfragen: bltype=0 (alle Performances) + bltype=1 (best per Athlet)
      for (const bltype of [0, 1]) {
        const url = buildUrl(year, cat, disc, indoor, bltype);
        await page.goto(url, { waitUntil: 'networkidle', timeout: 30000 });
        await page.waitForTimeout(1200);
        const rows = await parseTableForFiona(page, disc, indoor, year);
        if (rows.length > 0) console.log(`    bltype=${bltype}: ${rows.length} Fiona-Resultate`);
        allResults.push(...rows);
        found += rows.length;
      }
      if (found === 0) console.log(`    — keine Resultate`);
    }
  }
  ...
  console.log(`\n📊 Alle Fiona-Resultate (${unique.length}):`);
