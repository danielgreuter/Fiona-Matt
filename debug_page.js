...
async function pfSelect(page, inputId, labelText, partial = false) {
  ...
  console.warn(`  ⚠ "${labelText}" nicht gefunden. Verfügbar: ${
    avail.map(s => (s || '').trim()).filter(Boolean).slice(0, 10).join(' | ')
  }`);
...
(async () => {
  ...
  // 👉 WICHTIG: Anzeigen klicken
  try {
    await page.getByRole('button', { name: 'Anzeigen' }).click();
    console.log('  ✓ Anzeigen geklickt');
  } catch {
    console.warn('  ⚠ Anzeigen-Button nicht per Role gefunden → Fallback');
    await page.locator('button:has-text("Anzeigen"), input[value="Anzeigen"]').first().click();
    console.log('  ✓ Anzeigen geklickt via fallback');
  }

  // warten bis Resultate geladen sind
  await wait(5000);
