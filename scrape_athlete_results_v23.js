// scrape_athlete_results_v23.js
// Fix: onclick-Attribut des Buttons direkt ausführen (korrekte PrimeFaces-Parameter)
...
async function clickAnzeigen(page) {
  await page.waitForFunction(
    () => !document.querySelector('.ui-blockui, .ui-widget-overlay'),
    { timeout: 8000 }
  ).catch(() => {});
  ...
  const result = await page.evaluate(() => {
    ...
    if (!btn) return { status: 'not-found', onclick: null };

    const onclick = btn.getAttribute('onclick') || '';

    // Den echten onclick ausführen — enthält korrektes PrimeFaces.ab({s,f,p,u,...})
    if (onclick && onclick.includes('PrimeFaces')) {
      try {
        // eslint-disable-next-line no-eval
        const fn = new Function(onclick);
        fn.call(btn);
        return { status: 'onclick-executed', onclick: onclick.substring(0, 300) };
      } catch(e) {
        return { status: 'onclick-error: ' + e.message, onclick };
      }
    }

    // Fallback: direkter click
    btn.click();
    return { status: 'dom-click', onclick };
  });

  console.log(`    🔘 ${result.status}`);
  if (result.onclick) console.log(`    📎 onclick: ${result.onclick}`);
...
async function parseTable(page, discLabel, year) {
  await page.waitForFunction(
    () => {
      ...
      return !second.includes('Bitte') && !second.includes('wählen');
    },
    { timeout: 10000 }
  ).catch(() => {});
  ...
  const info = await page.evaluate(() => {
    ...
    return {
      count: trs.length,
      first4: trs.slice(0,5).map(r => r.textContent.trim().replace(/\s+/g,' ').slice(0,120)),
    };
  });
  console.log(`    → ${info.count} Zeilen`);
  if (info.count > 2) {
    info.first4.slice(1,4).forEach((r,i) => { if(r) console.log(`    [${i+1}]: ${r}`); });
  }

  const secondRowText = info.first4[1] || '';
