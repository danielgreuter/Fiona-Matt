// scrape_athlete_results_v27.js
// Direktnavigation zu alabus mit URL-Parametern — keine Dropdowns, kein Button-Click
...
// IDs aus onclick-Analyse (stabil, ändern sich nicht mit Jahr/Saison)
const DISC_IDS = {
...
async function parseTable(page, disc, year, indoor) {
  // Warten bis Tabelle mit echten Daten geladen ist
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { disc, indoor } of COMBOS) {
      const label = `${disc} ${indoor ? 'Indoor' : 'Outdoor'}`;
      console.log(`\n  📋 ${label}...`);
      ...
      console.log(`    ✓ ${filtered.length} Resultate`);
