...
// Basis: v55 + v56: Bestenlisten zusaetzlich mit srb=1 abfragen (windunterstuetzte Resultate), nur Outdoor-Sprints; Dedupe faengt Ueberschneidungen
...
function buildSaUrl(con, year, cat, disc, indoor, srb) {
  const p = new URLSearchParams({
    mobile: 'false', blyear: String(year), con,
    blcat: CAT_IDS[cat], disci: DISC_IDS[disc], top: '30', srb: String(srb == null ? '0' : srb),
  });
...
async function scrapeQuery(outer, inner, con, year, cat, disc, indoor, srb) {
  const saUrl = buildSaUrl(con, year, cat, disc, indoor, srb);
...
async function getEvents(page, compId, cache) {
  ...
  const byEid = new Map();
  const addAll = (evs) => { let n = 0; for (const e of evs) { if (!byEid.has(e.eid)) { byEid.set(e.eid, e); n++; } } return n; };
  let firstHtml = null;
  const visited = new Set();
  const queue = [`${LAPORTAL_BASE}/Competitions/Details/${compId}`];
  // bis 8 Unterseiten: ?page=N und tab-/tagespezifische Details-Links aus der Seite selbst
  while (queue.length && visited.size < 8) {
    const u = queue.shift();
    if (visited.has(u)) continue;
    visited.add(u);
    let html;
    try { html = await lapGet(page, u); } catch { continue; }
    if (!firstHtml) firstHtml = html;
    addAll(parseEventsFromHtml(html));
    // weitere Details-Unterseiten derselben Veranstaltung entdecken (Tabs/Tage/Pagination)
    const subs = [...html.matchAll(/href="(\/Competitions\/Details\/[^"]*)"/g)]
      .map(m => m[1].replace(/&amp;/g, '&'))
      .filter(h => h.includes(String(compId)));
    for (const h of subs) {
      const full = LAPORTAL_BASE + h;
      if (!visited.has(full) && queue.length + visited.size < 8) queue.push(full);
    }
    // klassische Pagination zusaetzlich probieren (nur direkt nach Seite 1)
    if (visited.size === 1) {
      for (let pg = 2; pg <= 4; pg++) queue.push(`${LAPORTAL_BASE}/Competitions/Details/${compId}?page=${pg}`);
    }
  }
  const events = [...byEid.values()];
  if (firstHtml) {
    eventStats[compId] = {
      c100: (firstHtml.match(/100\s*m/gi) || []).length,
      cFrauen: (firstHtml.match(/frauen|u18w|wom/gi) || []).length,
      links: [...firstHtml.matchAll(/href="(\/Competitions\/[^"]+)"/g)]
        .map(m => m[1].replace(/&amp;/g, '&'))
        .filter(h => h.includes(String(compId)) && !/CurrentList|ResultList/.test(h))
...
function parseEntryBlock(htmlPart) {
  // Wind aus dem Blockkopf: "Wind: +1,4 m/s" (ein Lauf) oder
  // "Wind: I:+0,2, II:+2,3 m/s" (Gesamtergebnis, pro Lauf)
  const windSeg = (htmlPart.match(/Wind\s*:([^<]{0,160})/i) || [])[1] || '';
  const windMap = {};
  for (const m of windSeg.matchAll(/([IVX]+)\s*:\s*([+-]?\d+[.,]\d+)/g)) {
    windMap[m[1]] = parseFloat(m[2].replace(',', '.'));
  }
  let singleWind = null;
  if (!Object.keys(windMap).length) {
    const sw = windSeg.match(/([+-]?\d+[.,]\d+)/);
    if (sw) singleWind = parseFloat(sw[1].replace(',', '.'));
  }
  ...
  for (const e of segs) {
    ...
    // Lauf-Referenz wie "1./II" in einer der Zellen -> Wind des jeweiligen Laufs
    let wind = singleWind;
    for (const cell of fl) {
      const hm = String(cell).match(/^\d+\.\s*\/\s*([IVX]+)$/);
      if (hm && windMap[hm[1]] != null) { wind = windMap[hm[1]]; break; }
    }
    const entry = {
      rank,
      name:   fl[1] || '',
    ...
    if (wind != null && !isNaN(wind)) entry.wind = wind;
    out.push(entry);
  }
  return out;
}
...
// Alle Ergebnis-Bloecke einer Seite einzeln parsen (Vorlauf 1, Vorlauf 2, Finale, Gesamtergebnis, ...)
function parseAllBlocks(html) {
  const positions = [...html.matchAll(/blocktable/g)].map(m => m.index);
  if (positions.length <= 1) {
    const b = parseEntryBlock(html);
    return b.length ? [b] : [];
  }
...
// Resultate vergleichen: '12,18'/'12.18'/'12.18 PB' -> gleich; numerische Toleranz 0.011
function resEq(a, b) {
  const na = String(a || '').replace(',', '.').match(/\d+(?::\d+)?(?:\.\d+)?/);
  const nb = String(b || '').replace(',', '.').match(/\d+(?::\d+)?(?:\.\d+)?/);
  if (!na || !nb) return false;
  if (na[0] === nb[0]) return true;
  const toSec = (s) => s.includes(':') ? s.split(':').reduce((acc, p) => acc * 60 + parseFloat(p), 0) : parseFloat(s);
  return Math.abs(toSec(na[0]) - toSec(nb[0])) < 0.011;
}

function isFionaName(nm) { return /matt/i.test(nm) && /fiona/i.test(nm); }

async function fetchBlocks(page, verb, eid, compId) {
...
async function preloadExistingTop5(results) {
  try {
    ...
    results.forEach(r => {
      ...
      if (o) {
        // Konsistenz: Fionas Zeit im gespeicherten Top5 muss zum Resultat passen
        // (repariert frueher falsch zugeordnete Vorlauf/Finale-Bloecke)
        const fe = (o.top5 || []).find(e => e.fiona);
        if (fe && !resEq(fe.result, r.result)) return;
        r.top5 = o.top5;
        if (o.fionaRank != null) r.fionaRank = o.fionaRank;
        if (o.fieldSize != null) r.fieldSize = o.fieldSize;
        // Wind-Upgrade: Outdoor-Sprint-Top5 ohne Wind erneut anreichern
        // (alte Top5 bleiben als Fallback erhalten, falls es nicht klappt)
        const isSprint = /^\d+m$/i.test(String(r.discipline).replace(/\s+/g, ''));
        const hasOwnWind = r.wind !== '' && r.wind != null;
        const t5HasWind = (o.top5 || []).some(e => e.wind != null);
        if (isSprint && hasOwnWind && !t5HasWind) r._windUpgrade = true;
        n++;
      }
    });
    return n;
...
async function enrichWithTop5(page, results) {
  ...
  const dated = results.filter(r => r.dateISO && (!(r.top5 && r.top5.length) || r._windUpgrade));
  ...
  for (const r of results) {
    ...
    if (r.top5 && r.top5.length && !r._windUpgrade) continue;
    ...
    try {
      ...
      for (const comp of comps) {
        ...
        // 2 Durchgaenge: erst nur Bloecke, in denen Fionas Zeit EXAKT dem Resultat entspricht,
        // dann (falls nichts passt) beliebiger Fiona-Block als Notnagel.
        for (const exactPass of [true, false]) {
          for (const c of cands) {
            const cacheKey = `${c.verb}/${c.eid}/${c.compId}`;
            let blocks = listCache[cacheKey];
            if (!blocks) {
              blocks = await fetchBlocks(page, c.verb, c.eid, c.compId);
              listCache[cacheKey] = blocks;
            }
            let fblocks = blocks.filter(b => b.some(e => isFionaName(e.name)));
            if (!fblocks.length) continue;
            if (exactPass) {
              fblocks = fblocks.filter(b => {
                const fe = b.find(e => isFionaName(e.name));
                return resEq(fe.result, r.result);
              });
              if (!fblocks.length) continue;
            }
            // groessten Block bevorzugen (Gesamtergebnis vor Einzellauf)
            fblocks.sort((a, b) => b.length - a.length);
            const entries = fblocks[0];
            const top5 = entries.slice(0, 5).map(e => {
              const o = { rank: e.rank, name: e.name, club: e.club, result: e.result };
              if (e.wind != null) { o.wind = e.wind; if (e.wind > 2.0) o.windAssisted = true; }
            ...
            ok++;
            found = true;
            console.log(`   ✅ ${r.discipline} ${r.date} (${comp.ort}): Rang ${r.fionaRank}/${entries.length}${exactPass ? '' : ' (ohne Zeit-Match)'}`);
            break;
          }
          if (found) break;
        }
      ...
      if (!found) {
        ...
        for (const comp of comps.slice(0, 4)) {
          ...
          const detail = cands2.slice(0, 4).map(c => {
            ...
            const n = listCache[k] ? listCache[k].reduce((s, b) => s + b.length, 0) : '?';
            return `"${c.name}"(${n})`;
          }).join(', ');
          ...
          if (!cands2.length && evs.length) {
            ...
            const st = eventStats[comp.compId];
            if (st) console.log(`         HTML-Stat: "100m"x${st.c100}, Frauen-Marker x${st.cFrauen}, Unterseiten: ${st.links.join(' , ') || 'keine'}`);
          }
        }
      }
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { disc, indoor } of QUERIES) {
      ...
      // Outdoor-Sprints: zusaetzlich windunterstuetzte Resultate (srb=1) einsammeln.
      // Ueberschneidungen entfernt der Dedupe (disc|date|result) weiter unten.
      if (!indoor && /\dm$/.test(disc.replace(/\s+/g, ''))) {
        const known = new Set(allResults.map(x => `${x.discipline}|${x.date}|${x.result}`));
        const rows2 = await scrapeQuery(outer, inner, FALLBACK_CON, year, cat, disc, indoor, '1');
        const neu = rows2.filter(x => !known.has(`${x.discipline}|${x.date}|${x.result}`));
        if (neu.length) {
          console.log(`    → +${neu.length} windunterstuetzt (srb=1)`);
          neu.forEach(r => console.log(`      💨 ${r.result} (${r.wind || '?'}) | ${r.date} | ${r.competition}`));
        }
        allResults.push(...rows2);
