// scrape_athlete_results_v17.js
// Fix: Anzeigen-Button via JS evaluate (alle möglichen Selektoren)
// + waitForFunction bis Tabelle > 2 Zeilen hat
// + HTML-Dump nach Anzeigen für Diagnose
...
// Anzeigen-Button via JS — findet alle möglichen Button-Varianten
async function clickAnzeigen(page) {
  const info = await page.evaluate(() => {
    const candidates = [
      ...
      ...document.querySelectorAll('a.ui-commandlink'),
    ];
    const btn = candidates.find(el =>
      (el.value || el.textContent || '').toLowerCase().includes('anzeig')
    );
    if (btn) {
      const info = `${btn.tagName} value="${btn.value}" text="${btn.textContent?.trim()}" id="${btn.id}" class="${btn.className?.slice(0,40)}"`;
      btn.click();
      return 'clicked: ' + info;
    }
    // Fallback: alle Buttons anzeigen
    return 'not found. Buttons: ' + candidates.map(el =>
      `[${el.tagName} "${(el.value||el.textContent||'').trim().slice(0,20)}"]`
    ).join(', ');
  });
  console.log(`  Anzeigen: ${info}`);
  if (info.startsWith('clicked')) {
    await page.waitForLoadState('networkidle', { timeout:12000 }).catch(()=>{});
    await page.waitForTimeout(2000);
  }
  return info.startsWith('clicked');
}
...
async function parseTable(page, discLabel, year) {
  // Warte bis Tabelle > 2 Zeilen oder 5s timeout
  ...
  const debug = await page.evaluate(() => {
    ...
    return {
      rowCount: trs.length,
      first3: trs.slice(0,3).map(r => r.textContent.trim().replace(/\s+/g,' ').slice(0,80)),
    };
  });
  console.log(`  → ${debug.rowCount} Zeilen. First3: ${JSON.stringify(debug.first3)}`);

  if (debug.rowCount <= 2) return [];
  ...
  return rows.map(cols => {
    ...
    return {
      ...
      result: resultCol, numResult: parseFloat(resultCol.replace(',','.')),
      wind: windCol||null, windAssisted: !isNaN(windNum)&&windNum>2.0,
      indoor: false,
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { season, disc } of COMBOS) {
      ...
      await selectPF(page,'form_anonym:bestlistCategory', cat, true);
      ...
      console.log(`  ✓ ${rows.length} Resultate für ${year}`);
