// scrape_athlete_results_v20.js
// Fix: [id="form_anonym:loadDataBtn"] statt #form_anonym\\:loadDataBtn (CSS-Doppelpunkt-Problem)
...
async function parseTable(page, discLabel, year) {
  await page.waitForFunction(
    () => {
      ...
      // Sicherstellen dass es keine "Bitte auswählen"-Meldung ist
  ...
  // "Bitte auswählen"-Zeile = keine echten Daten
  const secondRowText = info.first3[1] || '';
  if (secondRowText.includes('Bitte') || secondRowText.includes('wählen')) return [];
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const { season, disc } of COMBOS) {
      ...
      // *** FIX: [id=...] statt #id\\:subid — Doppelpunkt-Problem in CSS-Selektor ***
      try {
        const btn = page.locator('[id="form_anonym:loadDataBtn"]');
        ...
        console.log(`    ⚠ loadDataBtn: ${e.message.split('\n')[0]}`);
        // Debug-Screenshot bei Fehler
        await page.screenshot({ path: `debug_err_${year}_${DISC_MAP[disc]}_${season}.png` }).catch(()=>{});
      }
